...
export const formatCountdown = (date: Date): string => {
  ...
  try {
    ...
    const secondsLeft = Math.round(diff.seconds) as number;

    if (daysLeft === 0)
      return `${hoursLeft}h ${Math.round(minutesLeft)}m ${secondsLeft}s`;
    if (daysLeft < 28)
...
export const formatSkuCountdown = (date: Date): string => {
  //Iso to local time
  try {
    const startDayInLocal = DateTime.fromISO(date.toISOString()); //ISO-8601
    const now = DateTime.now();
